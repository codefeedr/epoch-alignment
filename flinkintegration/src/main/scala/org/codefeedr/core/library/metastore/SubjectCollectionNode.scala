...
trait SubjectCollectionNodeComponent extends ZkCollectionNodeComponent {
  this: ZkClientComponent with SubjectNodeComponent =>
  class SubjectCollectionNodeImpl(parent: ZkNodeBase)
      extends ZkCollectionNodeImpl[SubjectNode, Unit](
        "Subjects",
        parent,
        (name, parent) => new SubjectNodeImpl(name, parent))
      with SubjectCollectionNode {
