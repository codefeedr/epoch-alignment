...
class ProjectGeneratorSpec extends FlatSpec {
  ...
  "ProjectGenerator" should "Generate the same element with the same seed" in {
    assert(new ProjectGenerator(10,Some(eventTime)).generate() == new ProjectGenerator(10,Some(eventTime)).generate())
  }

  it should "Generate different elements with different seeds" in {
    assert(new ProjectGenerator(10,Some(eventTime)).generate() != new ProjectGenerator(11,Some(eventTime)).generate())
  }
}
