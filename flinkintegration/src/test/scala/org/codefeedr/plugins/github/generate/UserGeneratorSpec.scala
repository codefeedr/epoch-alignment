...
class UserGeneratorSpec extends FlatSpec {
  ...
  "UserGenerator" should "Generate the same element with the same seed" in {
    assert(new UserGenerator(10,Some(eventTime)).generate() == new UserGenerator(10,Some(eventTime)).generate())
  }

  it should "Generate different elements with different seeds" in {
    assert(new UserGenerator(10,Some(eventTime)).generate() != new UserGenerator(11,Some(eventTime)).generate())
  }
}
