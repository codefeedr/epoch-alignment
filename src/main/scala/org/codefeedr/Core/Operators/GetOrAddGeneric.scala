...
/**
  ...
  */
abstract class GetOrAddGeneric[A: ClassTag, B: ClassTag]() extends AsyncFunction[A, B] {
  ...
  /**
    ...
    */
  override def asyncInvoke(input: A, resultFuture: ResultFuture[B]): Unit = async {
    ...
    col
      .find(equal(GetIndexName, GetIndexValue(input)))
      .first()
      .subscribe(new Observer[B] {
        ...
        override def onComplete(): Unit = {
          ...
          resultFuture.complete(Iterable(output))
        }
  ...
  /**
    ...
    */
  def GetIndexValue(input: A): String
